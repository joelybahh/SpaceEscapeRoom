...
public class SE_Announcer : MonoBehaviour {
    ...
    [SerializeField]
    private List<SE_Announcement> m_timedAnnouncements;
    ...
    void Start()
    {
        ...
        m_timedAnnouncements = new List<SE_Announcement>();
    ...
    void Update()
    {
        ...
        for (int i = 0; i < m_timedAnnouncements.Count; i++)
        {
            ...
            if ((int)m_timedAnnouncements[i].m_minutes == (int)m_o2Timer.m_o2Clock.Minutes)
            {
                // check if the seconds are also the same
                if ((int)m_timedAnnouncements[i].m_seconds == (int)m_o2Timer.m_o2Clock.Seconds)
                {
